    ...
    <div className="flex flex-col min-h-screen">
      ...
      <section className="relative h-screen flex items-end overflow-hidden bg-[#2a2d3a]">
        ...
        <div className="container mx-auto px-4 z-20 relative pb-24 md:pb-32">
          ...
          >
            ...
            <div className="mb-6">
              ...
              <div className="w-8 h-0.5 bg-[#e8941a] mt-2" />
            </div>
            ...
            <Link
              href="/services"
              className="inline-flex items-center text-[#e8941a] font-bold text-lg hover:gap-3 gap-2 transition-all group"
            >
              {t("home.ourServices")}
        ...
        <div className="absolute bottom-10 left-1/2 -translate-x-1/2 z-20 flex items-center gap-2">
          ...
          <div className="w-6 h-2 rounded-full bg-[#e8941a]" />
          <div className="w-2 h-2 rounded-full bg-white/40" />
          <div className="w-2 h-2 rounded-full bg-white/40" />
        </div>
      </section>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {[
            ...
            <div key={i} className="flex items-start gap-4 group">
              <div className="text-[#e8941a] shrink-0 mt-1">{item.icon}</div>
              <div>
        ...
        <div className="container mx-auto px-4">
          <div className="flex justify-between items-end mb-12">
            <SectionHeader
              ...
              className="mb-0"
            />
            <Link href="/services" className="hidden md:flex items-center text-[#e8941a] font-bold hover:gap-3 gap-2 transition-all text-sm">
            ...
            {featuredServices?.map((service) => (
              <Card key={service.id} className="group border-none shadow-md overflow-hidden hover:shadow-xl transition-all duration-300 bg-white rounded-none">
                ...
                <CardContent className="p-6">
                  <h3 className="text-xl font-bold text-[#2a2d3a] mb-2 group-hover:text-[#e8941a] transition-colors">
                    {service.title}
                  </h3>
                  <p className="text-gray-500 line-clamp-3 mb-4 text-sm leading-relaxed">
                    {service.description}
                  </p>
                  <Link href="/services" className="inline-flex items-center text-sm font-bold text-[#e8941a] hover:gap-2 gap-1 transition-all">
          ...
          <div className="mt-8 text-center md:hidden">
            <Link href="/services" className="inline-flex items-center text-[#e8941a] font-bold gap-2">
              {t("home.viewAllServices")} <ArrowRight className="h-4 w-4" />
            </Link>
          </div>
        </div>
        ...
        <div className="container mx-auto px-4 relative z-10">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            <div>
              ...
              <div className="w-8 h-0.5 bg-[#e8941a] mt-2 mb-6" />
              ...
              <ul className="space-y-3 mb-8">
                {[t("home.isoCertified"), t("home.ecoFriendly"), t("home.yearsExp"), t("home.nationalCoverage")].map((item, i) => (
                  <li key={i} className="flex items-center text-gray-300">
                    <CheckCircle2 className="text-[#e8941a] h-4 w-4 mr-3 shrink-0" />
                    {item}
                  </li>
                ))}
              </ul>
              <Link
                href="/about"
                className="inline-flex items-center text-[#e8941a] font-bold hover:gap-3 gap-2 transition-all"
              >
            ...
            {featuredProjects?.map((project) => (
              <div key={project.id} className="group relative h-80 overflow-hidden cursor-pointer">
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/30 to-transparent flex flex-col justify-end p-6">
                  <span className="text-[#e8941a] text-xs font-bold tracking-wider mb-1">{project.location}</span>
                  <h3 className="text-white font-bold text-lg leading-tight group-hover:text-[#e8941a] transition-colors">
                    {project.title}
                  </h3>
                </div>
              </div>
      ...
      {/* CTA Section */}
      <section className="py-20 bg-[#e8941a]">
        <div className="container mx-auto px-4 text-center">
          <h2 className="text-3xl md:text-5xl font-black text-white mb-4 tracking-tight">{t("home.ctaTitle")}</h2>
          ...
          <div className="flex flex-col sm:flex-row justify-center gap-4">
            <Link
              href="/contact"
              className="inline-flex items-center justify-center bg-[#2a2d3a] text-white hover:bg-[#1e2230] font-bold text-base px-8 py-4 transition-colors"
            >
              {t("home.getInTouch")} <ArrowRight className="ml-2 h-4 w-4" />
            </Link>
            <Link
              href="/products"
              className="inline-flex items-center justify-center border-2 border-white text-white hover:bg-white hover:text-[#e8941a] font-bold text-base px-8 py-4 transition-colors"
            >
